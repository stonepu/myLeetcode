...
public class NC88 {
    ...
    public int findKth(int[] a, int n, int K) {
        // write code here
        quickFindKth(a, K, 0, n - 1);
        return kth;

    }

    public void quickFindKth(int[] a, int k, int head, int tail) {
        if(head > tail) return;
        if(head == tail) {
            if(head == a.length - k) {
                find = true;
        ...
        int tempHead = head;
        int tempTail = tail;
        while(tempHead < tempTail && !find) {
            while(tempHead < tempTail && a[tempHead] < a[tail]) {
                ++tempHead;
            }

            while(tempTail > tempHead && a[tempTail] >= a[tail]){
                --tempTail;
            }

            if(tempTail == tempHead) break;
            swap(a, tempHead, tempTail);

        }
        swap(a, tempHead, tail);
        if(tempHead == a.length - k) {
            find = true;
            kth = a[tempHead];
            return;
        }
        quickFindKth(a, k, head, tempHead - 1);
        quickFindKth(a, k, tempHead + 1, tail);
